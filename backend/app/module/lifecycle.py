...
class LifeCycleManager:
    ...
    async def startup(self):
        ...
        try:
            if gc_manager is not None:
                ...
                async def _gc_loop():
                    logger.info('GC periodic task started (interval=%s sec)', interval)
                    try:
                        while True:
                            try:
                                ...
                                logger.exception(f'Error in GC periodic task: {e}')
                    finally:
                        logger.info('GC periodic task stopped')

                self._gc_task = asyncio.create_task(_gc_loop())
        except Exception as e:
            logger.warning(f'Failed to start GC periodic task: {e}')
        ...
        try:
            if getattr(self, '_gc_task', None):
                ...
                try:
                    ...
                    logger.info('GC periodic task cancelled successfully')
        except Exception as e:
            logger.warning(f'Error cancelling GC task: {e}')

        # Shutdown FastEmbed process pool if any
        try:
            await embeddings.shutdown_fe_executor()
        except Exception:
            logger.debug('No FE executor to shut down or shutdown failed')
    ...
    def _connect_qdrant(self):
        ...
        if settings.qdrant_url:
            try:
                ...
                logger.error(f"Failed to connect to Qdrant URL: {e}")
                self._qdrant_client = None
        elif settings.qdrant_path:
            try:
                self._qdrant_client = QdrantClient(path=settings.qdrant_path)
                logger.info(f"Connected to local Qdrant at {settings.qdrant_path}")
            except Exception as e:
                logger.error(f"Failed to initialize local Qdrant: {e}")
                self._qdrant_client = None
        ...
        if self._qdrant_client:
            ...
            try:
                ...
                logger.info("Collection '%s' contains %s points", settings.qdrant_collection, count.count)
            except Exception:
                logger.debug('Could not retrieve collection count; qdrant may not expose count')
    ...
    def _ensure_collection(self):
        ...
        should_recreate = getattr(settings, 'qdrant_force_recreate', False)
        
        if should_recreate:
             logger.info('Force recreate enabled; recreating collection (this will delete existing data)')
             try:
                 ...
                 logger.info(f"Collection '{settings.qdrant_collection}' recreated.")
                 return
             except Exception as e:
